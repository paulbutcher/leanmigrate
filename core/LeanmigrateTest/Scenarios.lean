...
private def tableExists [SqlBackend Conn] (conn : Conn) (id : String) : IO Bool := do
  try
    discard <| SqlBackend.queryText1 conn s!"SELECT id FROM widget_{id}"
    pure true
  catch _ => pure false
...
def scenarioApplyThenRollback [SqlBackend Conn] (conn : Conn) (dir : System.FilePath) :
  ...
  check "apply then rollback: the latest's table is gone" !(← tableExists conn "00000002")
  check "apply then rollback: the earlier table remains" (← tableExists conn "00000001")
...
def scenarioFailureMidBatchAborts [SqlBackend Conn] (conn : Conn) (dir : System.FilePath) :
  ...
  check "the migration after the failure never ran" !(← tableExists conn "00000003")
